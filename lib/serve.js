...
const { jsWatch: jsWatchSetup } = require('./watch-js.js');
...
module.exports = function(srcDir, pubDir, options) {
  ...
  const { logLevel, browserSyncOptions } = options;
  ...
  const jsWatch = jsWatchSetup(srcDir, pubDir, null, options);
  const jsWatching = new Promise((resolve, reject) => {
    try {
      jsWatch(resolve, srcFile => {
        if (jsCompilerReady) {
  ...
  function srcController(req, res, next) {
    ...
    // bail if reqFile is not src, or if it is *minified* src
    ...
    let memFile = false;
    try {
      memFile = memoryFs.statSync(outFile).isFile();
    } catch (err) {
      /* pennyLogger.debug(`File not in memoryFs: {magenta:${reqFile}}`); */
    }
    if (memFile) {
