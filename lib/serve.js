...
const { bufferCache, memoryFs, syncFileTree, $data, $dataSyncer, $pagesSyncer } = require('./caches.js');
  ...
  function srcController(req, res, next) {
    ...
    if (reqExt == '.js') {
      if (outFile in bufferCache) {
        setHeaders(res, '.js');
        pennyLogger.debug(`Served from bufferCache: {magenta:${relative(srcDir, outFile)}}`);
