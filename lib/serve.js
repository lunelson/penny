...
module.exports = function(srcDir, pubDir, options) {
  ...
  if (!useHTTPS) bsync.use(inject);

  const bsyncRefresh = _.debounce(outExts => {
    if (allReady) {
      if (outExts.length == 1 && outExts[0] == '.html' && !useHTTPS) {
  ...
  function srcController(req, res, next) {
    ...
    if (outExt == '.js') {
      if (outFile in bundleCache) {
        ...
        pennyLogger.debug(`Served from bundleCache: {magenta:${relative(pubDir, outFile)}}`);
