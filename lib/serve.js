...
module.exports = function(srcDir, options) {
  ...
  const pageWatching = new Promise((resolve, reject) => {
    try {
      bsync.watch(['**/*.(pug|md|mdown|markdown|html)'], {
        ignored: ['**/node_modules/**', '_data/**', '**/_*.*'], // disallow only _file, not _folder/file
        cwd: srcDir
      }, (fsEvent, relFile) => {
        if (!~fileEvents.indexOf(fsEvent)) return;
  ...
  const srcWatching = Promise.all([ dataWatching, pageWatching ]).then(() => {
    try {
      return bsync.watch(['**/*'], {
        ...
        const isCompilerSrc = anymatch(['**/*.(pug|md|mdown|markdown|scss)'], relFile);
        const isHiddenFile = anymatch(['**/_*/**/*.*', '**/_*.*'], relFile);
        if (isCompilerSrc && !isHiddenFile) {
        ...
        if (srcCompilersReady) {
          ...
          // TODO: get an array of outExts, based on whatever depFiles are matched
          // const depFile = srcFile;
          // const depOutExtsSet = srcCompilers.reduce((set, compiler) => {
          //   if (compiler.check(depFile)) {
        ...
        pennyLogger.debug(`refreshing ${outExt} from srcWatching`);
        bsyncRefresh(outExt);
