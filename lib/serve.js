...
const { outFileCache, memoryFs } = require('./cache.js');
...
module.exports = function(srcDir, options) {
  ...
  // JS VARS
  const jsMultiCompiler = require('./compile-js')(srcDir);
  const jsFiles = new Set();
  jsMultiCompiler.init(jsFiles);
  const restartJsCompiler = _.debounce(() => jsMultiCompiler.restart(), 200);

  // PUG/SASS VARS
  let srcCompilersReady = false;
  const srcCompilers = new Map();
  function mdCompiler(file) { return file; }
  function pugCompiler(file) { return file; }
  function scssCompiler(file) { return file; }
  function syncSrcCompilers(event, file) {
    const ext = extname(file);
    if (event == 'unlink') {
      srcCompilers.delete(file);
      delete outFileCache[join(srcDir, file)];
      return;
    }
    if (event == 'add') {
      const srcCompiler = {
      ...
      srcCompilers.set(file, srcCompiler(file));
      return;
    }
  }
  ...
  const pageWatching = new Promise((resolve, reject) => {
    bsync.watch(['**/*.(pug|md|html)'], {
      ...
      syncFileDict($pages, event, file, file => matter(join(srcDir, file)));
      bsyncReload('.html');
    }).on('ready', resolve);
  });
  ...
  const jsWatching = new Promise((resolve, reject) => {
    bsync.watch(['**/*.js'], {
      ...
      if (event == 'add' || event == 'unlink') {
        ...
        event == 'unlink' && delete outFileCache[join(srcDir, file)];
        if (jsMultiCompiler.watching) restartJsCompiler();
      }
      bsyncReload('.js');
    }).on('ready', () => { resolve(); jsMultiCompiler.start(); });
  });
  ...
  const srcWatching = new Promise((resolve, reject) => {
    bsync.watch(['**/*.(pug|scss|md)'], {
      ...
      if (srcCompilersReady) {
        srcCompilers.forEach(srcCompiler => srcCompiler.checkDeps(file));
        bsyncReload(outExt);
      }
  ...
  Promise.all([dataWatching,pageWatching,jsWatching,srcWatching,bsyncRunning]).then(() => allReady = true);
  ...
  function srcController(req, res, next) {
    ...
    // FIND in outFileCache
    const reqFile = join(srcDir, pathname);
    let srcFile = reqSrcExts[ext].map(srcExt => replaceExt(reqFile, srcExt)).find(srcFile => srcFile in outFileCache);
    if (srcFile) {
      pennyLogger.info(`found in outFileCache: {magenta:${relative(srcDir, srcFile)}}`);
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('Content-Type', extContentTypes[extname(srcFile)]);
      return outFileCache[srcFile].then(buffer => toStream(buffer).pipe(res));
    }
    ...
    if (srcFile) {
      pennyLogger.info(`found in memoryFs: {magenta:${relative(srcDir, srcFile)}}`);
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('Content-Type', extContentTypes[extname(srcFile)]);
