...
module.exports = function(srcDir, options) {
  ...
  if (isDev) {
    ...
    bsync.watch(['_data/**/*.(json|yml|yaml)'], {
      ...
      const objPath = file.replace(/\.(json|yml|yaml)$/,'').split('/').slice(1);
      switch (event) {
      case 'unlink':
        _.unset(DATA, objPath);
        while (--objPath.length && _.isEmpty(_.get(DATA, objPath))) {
          _.unset(DATA, objPath);
        }
        // console.log(`removal: ${file}`);
        // console.log(JSON.stringify(DATA));
        break;
      default:
        _.set(DATA, objPath, basename(file));
        // console.log(`add/update: ${file}`);
        // console.log(JSON.stringify(DATA));
        break;
      }
      if (bsyncReady) {
