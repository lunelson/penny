...
// new deal
const dataErrMap = new Map();
const srcErrMap = new Map();
let jsErr = null;
...
function osdUpdate(socket = bsync.sockets) {
  ...
  updateTimeout = setTimeout(() => {
    // TODO: parse out the error from jsErr || srcErr || dataErr
    const err =
      jsErr || srcErrMap.size
        ? Array.from(srcErrMap.entries())[srcErrMap.size - 1]
        : dataErrMap.size
          ? Array.from(dataErrMap.entries())[dataErrMap.size - 1]
          : null;
    socket.emit(`penny:osd:${err ? 'show' : 'hide'}`, {
      error: err,
      // recentChanges,
    });
    updateTimeout = null;
...
module.exports = async function(options) {
  ...
  const watchingData = watchData(options, (err, absDataFile) => {
    dataErrMap[err ? 'set' : 'delete'](absDataFile, err);
    if (dataErrMap.size) return osdUpdate();
  ...
  const watchingSrc = watchSrc(options, (err, absSrcFile) => {
    const compiler = srcCompilerMap.get(absSrcFile);
    srcErrMap[err ? 'set' : 'delete'](compiler, err);
    if (srcErrMap.size) return osdUpdate();

    // clear or reset compiler(s)
    const outExts = [],
      watchFileExt = path.extname(absSrcFile);
    [...srcCompilerMap.values()].forEach(compiler => {
      if (compiler.check(absSrcFile)) {
  ...
  const watchingJs = watchJs(options, (err, watchFile) => {
    jsErr = err;
    if (jsErr) return osdUpdate();
    bsReload('.js');
  });

  // NB: must be sure that all of these resolve!!
  const [online] = await Promise.all([isOnline(), watchingJs, watchingData, watchingSrc])
