...
function configCreator(srcDir, pubDir, options) {
  ...
  const configureRules = require('./config-webpack-rules.js')(srcDir, options);
  ...
  return function(relFile) {
    ...
    return {
      ...
      entry: `./${relFile}`,
      output: {
        path: dirname(resolve(pubDir, relFile)),
        filename: basename(relFile),
        publicPath: dirname(resolve('/', relFile)).replace(/([^/])$/,'$1/'),
      },
      ...
      module: { rules: configureRules(resolve(pubDir, relFile)) },
