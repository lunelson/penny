...
function errStatsHandler(err, stats) {
  ...
  if (stats) {
    ...
    if (stats.hasWarnings()) info.warnings.forEach(err => console.warning(err.toString()));
  }
}
...
function configCreator(srcDir, pubDir, options) {
  ...
  const { isDev, browsers, babelIncludes = [] } = options;

  var pathSep = sep; if (pathSep == '\\') pathSep = '\\\\';
  ...
  const cssLoading = [
    isDev ?
      { loader: 'vue-style-loader', options: { hmr: false, sourcemap: true } } :
  ...
  return function configureEntry(relFile) {
    ...
    return {
        ...
        rules: [
...

          {
            ...
            oneOf: [
              ...
              { resourceQuery: /^\?vue/, use: [...pugLoading] },
          ...
          // },
          {
            test: /\.vue$/,
            loader: 'vue-loader',
            options: { hotReload: false, optimizeSSR: false, }
          },
      ...
      plugins: [
        new VueLoaderPlugin(),
        !isDev && new MiniCssExtractPlugin({ filename: '[name].js.css', })
      ].filter(val => val),
