...
function configCreator(baseDir, outDir=null, options) {

  const { isDev, browsers } = options;

  return function configureEntry(relFile) {
    const srcFile = join(baseDir, relFile);
    const outFile = outDir?join(outDir, relFile):srcFile;
    const sassFns = sassFunctions(baseDir, srcFile);
    return {
      ...
      output: {
        ...
        publicPath: `/${dirname(relative(baseDir, srcFile))}/`,
      },
