...
const configurePostCSS = require('./config-postcss');

// export
module.exports = function (srcDir, pubDir, options, depReporter) {
  const { isDev } = options;

  const postCSS = configurePostCSS(options);
  ...
  return class ScssCompiler extends SrcCompiler {
    constructor(srcFile) {
      super(srcFile);
      this.renderOptions = configureNodeSass(this.srcFile, this.depFiles);
    }
    ...
    stream() {
      try {
        if (!('outCache' in this)) {
          ...
          // POSTCSS RENDER
          this.outCache = postCSS.process(css, {
            from: file,
            to: outFile,
