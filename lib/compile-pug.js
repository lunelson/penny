...
module.exports = function(srcDir, options) {
  ...
  return class PugCompiler extends SrcCompiler {
    ...
    // check(absFile){
    //   if (super.check()) pugLogger.debug(`found dep: ${absFile}`);
    //   // pugLogger.debug('checking deps');
    //   // if (~this.depFiles.indexOf(absFile)) {
    //   //   pugLogger.debug(`found dep: ${absFile}`);
    //   //   this.reset(); }
    // }
    reset() {
      super.reset();
    ...
    error({message}) {
      if (!isDev) throw Error(message);
      pugLogger.error(message);
      return toStream(htmlCssErr(message));
    }
    stream() {
      try {
        if (!('template' in this)) {
        ...
        if (!('outCache' in this)) {
          ...
          pugLogger.debug(`depFiles logged: ${this.depFiles}`);
        }
