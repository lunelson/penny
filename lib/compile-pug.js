    ...
    stream() {
      try {
        if (!('template' in this)) {
          ...
          pugLogger.debug(`depFiles logged: ${this.depFiles.length}`);
        }
        if (!('outCache' in this)) {
          const { $page, options: $options, locals, locals: { $data, $pages } } = this;
          ...
          this.outCache = this.template(_.assign(locals, { $page, $options }));
        }
