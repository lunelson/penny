  ...
  return function(srcFile) {
    return {
      ...
      markdownFile(relFile){
        ...
        this._depFiles.push(absFile);
        return '\n'+markdown(content).trim();
      },
      pug(str) {
        try {
          const compiler = pug.compile(str, this.$options);
          this._depFiles.push(...compiler.dependencies);
          return '\n'+compiler(this).trim();
      ...
      pugFile(relFile) {
        ...
        try {
          ...
          this._depFiles.push(absFile, ...compiler.dependencies);
          return '\n'+compiler(this).trim();
        } catch(err) { throw Error(err); }
      },
