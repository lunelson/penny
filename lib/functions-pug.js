...
module.exports = function (srcDir, pubDir, options) {
  ...
  return function (srcFile, depFiles) {
    ...
    const locals = {
      ...
      markdown(str) {
        return '\n' + mdi.render(str.trim()).trim();
      },
      markdownInline(str) {
        return '\n' + mdi.renderInline(str.trim()).trim();
      },
      ...
      require(relModule) {
        ...
        /*
          NOTE: consider using a require module that does not cache
            https://www.npmjs.com/package/require-uncached
            https://www.npmjs.com/package/requirefresh
        */
