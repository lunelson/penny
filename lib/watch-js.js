...
function jsWatchSetup(srcDir, pubDir, outDir, options) {

  // 0. create a dummy event callback reference
  ...
  let onCompilerEvent;
  const compiler = JsCompiler(srcDir, pubDir, options);
  compiler.init(jsFiles, srcFile => onCompilerEvent(srcFile));
  const restartCompiler = _.debounce(() => compiler.restart(), debounceTime);
  ...
  return function(onReady, onEvent) {
    ...
    const watcher = chokidar.watch(['**/*.js'], {
      ignored: ['**/node_modules/**', '**/_*/**', '**/_*', '**/*.min.js'],
      cwd: pubDir
    }).on('ready', () => { watchReady.resolve(); compiler.start(); });
    ...
    watcher.on('all', (fsEvent, relFile) => {
      ...
      // resolve abs srcFile
      const srcFile = resolve(pubDir, relFile);
      ...
      if (compiler.watching) restartCompiler();
    });

    // make sure onReady gets the watcher, when everything is really ready
    Promise.all([watchReady, compiler.ready]).then(() => onReady(watcher));
...
module.exports = { jsFiles, jsWatch: jsWatchSetup };
