...
test('mdi only configs once', t => {
  t.is(mdi, mdi2);
});

test('block rendering', t => {
  t.snapshot(mdi.render('hello __world__'));
});

test('inline rendering', t => {
  t.snapshot(mdi.renderInline('hello __world__'));
});
