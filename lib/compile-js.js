...
function JsCompiler(srcDir, pubDir, options) {
  ...
  return {
    ...
    start(){
      ...
      /*
        const { monoConfig, multiConfig } = require('config-webpack');
        const isMono = options.isBuild && options.webpack.monoCompile
        this.config = (isMono ? monoConfig : multiConfig)(this.fileSet);
        this.compiler = webpack(this.config);
        !options.isBuild && this.compiler.outputFileSystem = memoryFs;
        (this.compiler.compilers||[this.compiler]).forEach(compiler => {
          // ...
        })
