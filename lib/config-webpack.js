...
exports.entryConfigurator = function(options) {
  return function(srcFile) {
    ...
    const subDir = path.dirname(path.relative(pubDir, srcFile));
    ...
    return {
      ...
      module: {
        ...
        rules: configureRules(srcFile, options),
      },
...
function configureRules(srcFile, options) {
  ...
  const configNodeSass = require('./config-scss.js')(srcFile, options);
    ...
    babelExclude != undefined
      ...
      : function(file) {
          return (
            ...
            !/\.vue\.js/.test(file) &&
            !babelInclude
              .map(moduleName => new RegExp('node_modules' + (path.sep == '\\' ? '\\\\' : path.sep) + moduleName))
