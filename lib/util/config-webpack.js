...
module.exports = function(baseDir, isDev, options) {
  ...
  const { browsers: browsers, linting } = options;
  const procDir = process.cwd();
  // const pennyDir = findup('node_modules', {cwd: __dirname, nocase: true});
  ...
  return function(srcFile) {
    ...
    return {
      ...
      resolve: {
        ...
        modules: [procDir, resolve(__dirname, '../../node_modules'), 'node_modules'], // ?? allow local modules to work? e.g. lodash
      },
