...
const boundFns = {
  ...
  // pug-ception
  render(str, opts) {
    try { return pug.render(str, Object.assign({}, this, opts, { filename: this.filename })); }
    catch(err) { throw Error('[pug] render(): something with inputString is no good'); }
  },
  renderFile(relFile, opts) {
    const absFile = relFile[0] === '/' ? path.join(this.basedir, relFile) : path.resolve(this.dirname, relFile);
    try { return pug.renderFile(absFile, Object.assign({}, this, opts, { filename: absFile })); }
    catch(err) { throw Error(`[pug] renderFile(): could not read file at '${absFile}`); }
  }
};
