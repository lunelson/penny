...
const boundFns = {
  ...
  imgInfo(relImg) { return imageSize(path.join(this.dirname, relImg)); },
  require(relFile) {
    const filePath = resolve.sync(relFile, { basedir: this.dirname });
    if (filePath.match(/\.yml$|\.yaml$/)) return readYaml.sync(filePath);
