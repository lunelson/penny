...
const htmlSrcExtGroup = `(${outSrcExts['.html'].map(ext => ext.slice(1)).join('|')})`;
const cssSrcExtGroup = `(${outSrcExts['.css'].map(ext => ext.slice(1)).join('|')})`;
