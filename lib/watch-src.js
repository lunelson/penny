      ...
      const watcher = chokidar
        ...
        .on('all', (fileEvent, relFile) => {
          ...
          if (isHtmlEntry && !isHidden && !isDynamic) {
            const route = '/' + relFile.replace(new RegExp(`\\.${htmlSrcExtGroup}$`), '.html');
            if (fileEvent == 'unlink') {
              ...
              try {
                ...
                staticRouteMap.set(
                  route,
                  Object.assign(value || {}, { route, srcFile: path.relative(srcDir, absFile) }),
                );
              } catch (err) {
                watchErr = err;
          ...
          if (isHtmlEntry && !isHidden && isDynamic) {
            const pattern =
              '/' + relFile.replace(new RegExp(`\\.${htmlSrcExtGroup}$`), '(.html)').replace(/\$/g, ':');
            ...
            if (fileEvent == 'unlink') {
              ...
              try {
                ...
                dynamicRouteMap.set(
                  pattern,
                  Object.assign(value || {}, { pattern, matcher, srcFile: path.relative(srcDir, absFile) }),
                );
              } catch (err) {
                watchErr = err;
                pennyLogger.error(`{red:${__basename}}: ${err.message}`, err);
            ...
            if (fileEvent == 'unlink') {
              ...
              try {
                ...
                if (validSrc) {
                  ...
                  srcCompilerMap.set(absFile, new SrcCompiler(absFile, options));
                }
              } catch (err) {
                watchErr = err;
                pennyLogger.error(`{red:${__basename}}: ${err.message}`, err);
